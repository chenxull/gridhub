	...
	if err != nil {
		...
		if !errs.IsStatusMismatchError(err) {
			...
			if er := bt.pushToQueueForRetry(targetStatus); er != nil {
				//todo logger.Errorf("push job status update request to retry queue error: %s", er)
...
// retrieve the stats of job tracked by this tracker from the backend data redis
...
func parseInt64(v string) int64 {
	...
	if err != nil {
		//todo logger.Errorf("Parse int64 error: %s", err)
		return 0
	}
	return intV
}
func closeConn(conn redis.Conn) {
	if conn != nil {
		if err := conn.Close(); err != nil {
			//todo logger.Errorf("Close redis connection failed with error: %s", err)
		}
	}
}
